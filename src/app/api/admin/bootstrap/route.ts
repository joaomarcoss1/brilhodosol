...
export async function GET(request: NextRequest) {
  ...
  try {
    const [branchesRes, employeesRes, notificationsRes] = await Promise.all([
      ...
      auth.supabase.from("admin_notifications").select("id,title,message,notification_type,read_at,created_at").is("read_at", null).order("created_at", { ascending: false }).limit(8)
    ]);
