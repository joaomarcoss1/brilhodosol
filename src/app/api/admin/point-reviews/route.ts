...
export async function PATCH(request: NextRequest) {
  ...
  if (!body.id || !["approved", "rejected", "adjusted", "cancelled", "pending_review"].includes(body.status)) {
  ...
  if (oldError || !oldData) return fail("Ocorrência não encontrada.", 404, oldError?.message);
